...
def get_ai_market_intelligence():
    ...
    1. 總結 3 條最重要的世界新聞。
    2. 分析哪些題材目前被看好。
    3. 分析哪些新聞可能導致哪些股票或板塊下跌。
    4. 推薦 3-5 個今日最值得關注的台股題材關鍵字。
    請用繁體中文回答，內容精簡，適合手機閱讀。
    """
    try:
        # 使用最穩定的生成方式
...
def run_full_scan(vol_limit, stock_pool):
    ...
    for i, symbol in enumerate(stock_pool):
        try:
            ...
            name_map = {"2330": "台積電", "2317": "鴻海", "2454": "聯發科", "2382": "廣達", "2603": "長榮", "3231": "緯創"}
...
with st.sidebar:
    ...
    stock_pool = ["2330.TW", "2317.TW", "2454.TW", "2382.TW", "3231.TW", "2603.TW", "2881.TW", "2303.TW", "2609.TW", "2618.TW"]

if st.button("🔍 執行全自動市場掃描", use_container_width=True):
    final_data = run_full_scan(vol_input, stock_pool)
    if not final_data.empty:
        st.subheader("🔥 掃描結果 (依照成交張數排序)")
